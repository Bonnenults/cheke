...
Article.jumpCreatePage = function() {
    if(!Index.isHashAuthor()) {
        return;
    }
    App.jump("/article/yxpt/create/createArticle.action");
 ...
 * @param articleId
 */
Article.publish = function(articleId) {
    mizhu.confirm('提示', '确认发布？', function(flag) {
        if(flag) {
            $.ajax({
                type: "POST",
                url:contextpath + '/article/yxpt/update/publishArticle.action',
                data: {id: articleId},
                dataType: "json",
                cache: false,
 ...
 * @param articleId
 */
Article.cancel = function(articleId) {
    mizhu.confirm('提示', '确认取消发布？', function(flag) {
        if(flag) {
            $.ajax({
                type: "POST",
                url: contextpath + '/article/yxpt/update/cancelPublishArticle.action',
                data: {id: articleId},
                dataType: "json",
                cache: false,
                success: function (data) {
 ...
 * @param articleId
 */
Article.offline = function(articleId) {
    mizhu.confirm('提示', '确认下线？', function(flag) {
        if(flag) {
            $.ajax({
                type: "POST",
                url: contextpath + '/article/yxpt/update/offLineArticle.action',
                data: {id: articleId},
                dataType: "json",
                cache: false,
                success: function (data) {
 ...
 * @param articleId
 */
Article.copyArticle = function(articleId) {
...
Article.batchPublish = function() {
    ...
    $.ajax({
        type: "POST",
        url: contextpath + '/article/yxpt/update/batchPublishArticle.action',
...
Article.batchCancel = function() {
    ...
    $.ajax({
        type: "POST",
        url: contextpath + '/article/yxpt/update/batchCancelArticle.action',
        data: {articleIds: articleIds},
        dataType: "json",
        cache: false,
        success: function (data) {
...
Article.batchOffline = function() {
    ...
    $.ajax({
        type: "POST",
        url: contextpath + '/article/yxpt/update/batchOfflineArticle.action',
        data: {articleIds: articleIds},
        dataType: "json",
        cache: false,
        success: function (data) {
...
Article.batchCopy = function() {
    ...
    $.ajax({
        type: "POST",
        url: contextpath + '/article/yxpt/update/batchCopyArticle.action',
        data: {articleIds: articleIds},
        dataType: "json",
        cache: false,
        success: function (data) {
 ...
 * 保存或修改文章
 */
Article.saveOrUpdate = function() {
    if(Article.validator.validate()) {
        ...
        if(tagIds.length <= 0) {
            mizhu.alert("请先选择一个资讯分类");
            return;
        }

        if($("#body").val() == '') {
            mizhu.alert("资讯正文不能为空");
            return;
        }
        ...
        mizhu.confirm('提示', remindMsg, function(flag) {
            if(flag) {
                jQuery.ajax({
                    type:"POST",
                    url:contextpath + '/article/yxpt/create/createOrUpdateArticle.action',
                    data:data,
                    dataType:"json",
                    ...
                    success:function(data){
                        if("ok"== data.code){
                            App.jump("/article/yxpt/list/listArticle.action");
                        }else{
                            mizhu.alert(data.msg);
                        }
                    }
...
Article.changeStatus = function(status) {
    $("#status").val(status);
    $("[name='type']").val("");
    $("[name='aIsTask']").val("");
    if(status == 0) { //待发布
...
Article.deleteImg=function(){
    $("#image").val("");
    $("#projectImg_span").html("");
    $("#projectImg_span_delete").html('');
}

Article.upload = function(n,fileSizeLimit) {
        $.ajaxFileUpload({
            ...
            success : function(data, status) {
                if(data.indexOf("http") != -1) {
                    var url = data;
                    html='<img src="'+url+'" /> <a href="javascript:;" onclick="Article.deleteImg()" class="image_juzhong" style="color: red">删除</a> ';
                    $("#image").val(url);
...
Article.validator = null;

Article.initArticle = function(){
    //添加验证
    var validator = new Clover.validator.Validator("#form_data");
    Article.validator = validator;
   ...
   /* //上传营业执照
    $("#projectImg").uploadify( {
        swf : contextpath + '/js/framework/upload/uploadify.swf',
        uploader : contextpath + '/upload/uploadTempImg.action',
        auto : true,
        buttonText : '上传图片',
        cancelImage : contextpath + '/js/framework/upload/uploadify-cancel.png',
        fileSizeLimit : 100,
        fileTypeDesc : '图片',
        width:84,
        height:24,
