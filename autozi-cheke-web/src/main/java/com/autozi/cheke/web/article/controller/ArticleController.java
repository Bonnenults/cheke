...
public class ArticleController extends BaseController{
    ...
    public String listArticle(Model uiModel,HttpServletRequest request,String ajax) throws Exception{
        ...
        filter.put("channelType", ArticleChannel.ZIXUN.getType());
        ...
        if(ajax != null) {
            return "/article/article/listArticle_ajax";
        }
        return "/article/article/listArticle.html";
    }
    ...
    public String createArticle(@RequestParam(required = false) Long id, Model uiModel) throws Exception{
        ...
        channel.put("channelType", ArticleChannel.ZIXUN.getType());

        uiModel.addAttribute("articleTag",articleFacade.findArticleTagList(channel,id));

        return "/article/article/createOrUpdateArticle.html";
    }
    ...
    public String getArticleDetail(@RequestParam(required = true)Long id,Model uiModel) throws Exception{
        ...
        return "/article/article/previewArticleDetail.html";
    }
    ...
    public void createOrUpdateArticle(ArticleVo articleVo,HttpServletResponse response) throws Exception{
        ...
        if(articleVo.getId() == null) {
            articleVo.setaIsTop(0);//首次添加设置排序为0
    ...
    public void publishArticle(Long id, HttpServletResponse response) throws Exception{
        ...
        if(!articleFacade.checkPublish5Num(ArticleChannel.ZIXUN.getType(),getCurrentUserId(),id)) {
            responseJson(response,"err","每天只能发布5篇资讯");
            return;
        }
    ...
    public void batchPublishArticle(String articleIds,HttpServletResponse response) throws Exception{
        ...
        if(listIds.size() > 5) {
            responseJson(response,"err","每次只能发布5篇资讯");
            return;
        }

        if(!articleFacade.checkBatchPublish5Num(ArticleChannel.ZIXUN.getType(),listIds.size(),getCurrentUserId(),listIds)) {
            responseJson(response,"err","每天只能发布5篇已发布过的资讯");
            return;
        }

        if(articleFacade.checkBatchPublishDate(listIds)) {
            responseJson(response,"err","截止日期应大于发布时期，请检查是否全部满足？");
            return;
        }
