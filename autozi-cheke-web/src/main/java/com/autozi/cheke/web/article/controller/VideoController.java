...
public class VideoController extends BaseController{
    ...
    public String listArticle(Model uiModel,HttpServletRequest request,String ajax) throws Exception{
        ...
        filter.put("channelType", ArticleChannel.VIDEO.getType());
        ...
        if(ajax != null) {
            return "/article/video/listArticle_ajax";
        }
        return "/article/video/listArticle.html";
    }
    ...
    public String createArticle(@RequestParam(required = false) Long id, Model uiModel) throws Exception{
        ...
        channel.put("channelType", ArticleChannel.VIDEO.getType());
        uiModel.addAttribute("articleTag",articleFacade.findArticleTagList(channel,id));

        return "/article/video/createOrUpdateArticle.html";
    }
    ...
    public String getArticleDetail(@RequestParam(required = true)Long id,Model uiModel) throws Exception{
        ...
        return "/article/video/previewArticleDetail.html";
    }
    ...
    public void publishArticle(Long id, HttpServletResponse response) throws Exception{
        ...
        if(!articleFacade.checkPublish5Num(ArticleChannel.VIDEO.getType(),getCurrentUserId(),id)) {
            responseJson(response,"err","每天只能发布5篇培训");
            return;
        }
    ...
    public void batchPublishArticle(String articleIds,HttpServletResponse response) throws Exception{
        ...
        if(listIds.size() > 5) {
            responseJson(response,"err","每次只能发布5篇培训");
            return;
        }

        if(!articleFacade.checkBatchPublish5Num(ArticleChannel.VIDEO.getType(),listIds.size(),getCurrentUserId(),listIds)) {
            responseJson(response,"err","每天只能发布5篇已发布过的培训");
            return;
        }
