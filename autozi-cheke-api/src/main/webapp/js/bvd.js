(function($) {
    ...
    var bvd = function(dom) {
		...
		$.ready(function() {
			...
			//获取视频父元素
			that.vRoom = that.video.parentNode.parentNode;
			//元素初始化
			that.initEm();
            //计算样式
    		that.initStyle();
        ...
        window.onresize = function(){
            ...
            that.vRoom.style.width = (this.vW = this.sw) + 'px';
            that.vRoom.style.height = (this.vH = vh) + 'px';
        };
	...
	pro.initEm = function() {
		...
		this.vC.innerHTML = '<div><span class="current">00:00</span></div><div><div class="progressBar"><div class="timeBar"><div class="crr"></div></div></div></div><div><span class="duration">00:00</span></div><div><div class="fill"><img src="images/fangda.png"/></div></div>';
...
//      }
	...
	pro.initEvent = function() {
        ...
        var video_bt = document.querySelector('.video_bt');
		var that = this;
		//给播放按钮图片添加事件
...
//		//视频手势左滑动事件
        ...
        this.vC.querySelector('.fill').addEventListener('tap', function() {
            that.video.pause();
            that.video.removeAttribute('webkit-playsinline');
            that.video.removeAttribute('playsinline');
            that.video.play();
            setTimeout(function(){
                that.video.setAttribute('webkit-playsinline','true');
                that.video.setAttribute('playsinline','true');
			},100)
            launchFullscreen(that.video);
	...
	pro.switch = function() {
        var vR = this.vRoom;
        var oDiv = document.querySelector('.bad-video');
        var oVideo = document.querySelector('video');
        if(this.oSwitch){
            oVideo.style.height = 'auto';
            oDiv.style.height = info[1]+'px';
            oVideo.style.marginTop = -oVideo.offsetHeight/2+'px';
            oVideo.style.top = '50%';
            oVideo.style.position = 'absolute';
            this.oSwitch = false;
        }else{
            oDiv.style.height = info[0]/ 3 * 2+'px';
            oVideo.style.height = '100%';
            oVideo.style.marginTop = 0;
            oVideo.style.top = '0';
            oVideo.style.position = 'static';
            this.oSwitch = true;
        }
        // $(function(){
        //     $('.bad-video').css('height',info[1]);
        //     $('video').css({'margin-top':-$('video').offsetHeight/2,'position':'absolute','top':'50%','height':'auto'});
        // })
