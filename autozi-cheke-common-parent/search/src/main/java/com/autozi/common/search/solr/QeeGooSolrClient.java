...
public class QeeGooSolrClient {
	...
	private static Logger logger = LoggerFactory.getLogger(QeeGooSolrClient.class);
...
//    private static SolrServer server;
    ...
    private static QeeGooSolrClient instance;

    private static QeeGooSolrClient collinstance;
    ...
    public static QeeGooSolrClient getInstance(){
    	if(instance==null){
//    		instance=new QeeGooSolrClient(url+SOLR_CORE[0]);
    		instance=new QeeGooSolrClient(url);
    	}

    	return instance;
    }
    
    public static QeeGooSolrClient getCollInstance(){
    	if(collinstance==null){
    		collinstance=new QeeGooSolrClient(url+SOLR_CORE[1]);
    	}
    ...
    private QeeGooSolrClient(String url) {
//        server = SolrServerFactory.getCommonsHttpSolrServer(url);
...
//        	for (IndexEntity indexEntity : entityList) {
//        		server.addBean(indexEntity);
//        	}
    ...
    public QeeGooSolrSearchResult findPageForProFilter(Page<IndexEntity> page, List<PropertiesFilter> propertiesFiltersList, String ownerPartyId) throws Exception{
        ...
        try {
            ...
            SolrQuery solrQuery = QeeGooSolrClientHelper.initSolrQuery(page, propertiesFiltersList, ownerPartyId);
            if(solrQuery!=null){
     ...
     * @return
     */
    public List<IndexEntity> findIndexEntityListforProfilter(List<PropertiesFilter> propertiesFiltersList, String ownerPartyId){
        List<IndexEntity> entityResult=null;
        SolrQuery solrQuery = QeeGooSolrClientHelper.initSolrQuery(propertiesFiltersList, ownerPartyId);
        try {
            QueryResponse response = server.query(solrQuery);
            DocumentObjectBinder binder = new DocumentObjectBinder();
            SolrDocumentList docResult = response.getResults();
            if(null != docResult && docResult.size() > 0){
                entityResult = binder.getBeans(IndexEntity.class, docResult);
            }
        } catch (SolrServerException e) {
            logger.error("根据Id："+solrQuery+"，查询失败:"+e.getMessage());
        }
        return entityResult;
    }
    ...
    public List<QeeGooFacetField> findFacetForProFilter(List<PropertiesFilter> propertiesFiltersList, String ownerPartyId){
        QueryResponse response=null;
        try {
            SolrQuery solrQuery = QeeGooSolrClientHelper.initSolrFacetQuery(propertiesFiltersList, ownerPartyId);
            response = server.query(solrQuery);
        } catch (Exception e) {
            logger.error("solr 查询异常:"+e.getMessage());
            e.printStackTrace();
     ...
     * 2015-12-1下午06:53:30
     */
    public QeeGooSolrSearchResult findPageForProFilterForNoLogin(Page<IndexEntity> page, List<PropertiesFilter> propertiesFiltersList, String ownerPartyId) throws Exception{
        QueryResponse response=null;
        QeeGooSolrSearchResult result = new QeeGooSolrSearchResult();
        logger.info("查询开始,开始计时");
        _logger_solr.info("查询开始,开始计时");
        ...
        try {
        	stopWatch.start();
            SolrQuery solrQuery = QeeGooSolrClientHelper.initSolrQueryForNoLogin(page, propertiesFiltersList, ownerPartyId);
            if(solrQuery!=null){
            	response = server.query(solrQuery);
            }
            stopWatch.stop();
     ...
     * @prop facetFieldList facet的结果
     * */
    public QeeGooSolrSearchResult findPageForProFilterForFlagshipStore(Page<IndexEntity> page, List<PropertiesFilter> propertiesFiltersList, String ownerPartyId) throws Exception{
        QueryResponse response=null;
        QeeGooSolrSearchResult result = new QeeGooSolrSearchResult();
        logger.info("查询开始,开始计时");
        _logger_solr.info("查询开始,开始计时");
        ...
        try {
            stopWatch.start();
            SolrQuery solrQuery = QeeGooSolrClientHelper.initSolrQueryForFlagshipStore(page, propertiesFiltersList, ownerPartyId);
            if(solrQuery!=null){
                response = server.query(solrQuery);
            }
            stopWatch.stop();
