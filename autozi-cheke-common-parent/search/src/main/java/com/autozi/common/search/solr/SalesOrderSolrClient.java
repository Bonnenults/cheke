 ...
 * 创建时间: 13-3-13 下午5:20
 */
public class SalesOrderSolrClient {
    private static final String SOLR_ZKHOST_SERVER_URL = "order.solr.zkhost.server.url";
	private static Logger logger = LoggerFactory.getLogger(SalesOrderSolrClient.class);
	private static Logger _logger_solr = LoggerFactory.getLogger("SOLR");
//    private static SolrServer server;
    private static CloudSolrServer server;
    private static final String DEFAULT_SOLR_CORE = "autozi_order";
    private static SalesOrderSolrClient instance;

    private static SalesOrderSolrClient collinstance;
    ...
    public static SalesOrderSolrClient getInstance(){
    	if(instance==null){
//    		instance=new QeeGooSolrClient(url+SOLR_CORE[0]);
    		instance=new SalesOrderSolrClient();
    	}

    	return instance;
    }
    
    public static SalesOrderSolrClient getCollInstance(){
    	if(collinstance==null){
    		collinstance=new SalesOrderSolrClient();
    	}
...
//        server = SolrServerFactory.getCommonsHttpSolrServer(url);
    ...
    public void addBeans(Collection<SalesOrderEntity> entityList){
        try {
...
//        	}
    ...
    public void addBean(SalesOrderEntity entity){
        try {
        	logger.info("=====提交索引数据=====开始");
//        	for (IndexEntity indexEntity : entityList) {
//        		server.addBean(indexEntity);
//        	}
     ...
     * */
    public QeeGooSolrSearchResult findPageForProFilter(Page<SalesOrderEntity> page, List<PropertiesFilter> propertiesFiltersList) throws Exception{
        ...
        try {
            ...
            SolrQuery solrQuery = SalesOrderSolrClientHelper.SolrQuery(page, propertiesFiltersList);
            if(solrQuery!=null){
        ...
        if(null != response){
            ...
            result.setSalesOrderEntityPage(page);
    ...
    private Page<SalesOrderEntity> assambleIndexEntityResult(Page<SalesOrderEntity> page, QueryResponse response) {
        SolrDocumentList solrDocList = response.getResults();
        DocumentObjectBinder binder = new DocumentObjectBinder();
        List<SalesOrderEntity> indexEntityList = binder.getBeans(SalesOrderEntity.class, solrDocList);
