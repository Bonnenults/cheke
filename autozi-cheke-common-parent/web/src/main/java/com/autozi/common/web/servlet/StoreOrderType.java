...
public class StoreOrderType {
	...
	public String StoreFile(String storeType, FileItem item, String today){
		...
		if("thumb".equals(storeType)){
			...
			name = storeFiles(item, today);
		}else if("files".equals(storeType)){
			name = storeFile(item, today);
		}else{
...
//					+ UUID.randomUUID().toString();
            try {
    			IFile file = FileManagerFactory.getAttDocManager().createFile(name);
	...
	private String storeFiles2Img_thumb(FileItem item, String today) {
		if (item.isFormField() == false) { 
            ...
            try {
                IFile file = FileManagerFactory.getAttDocManager().createFile(name_img);
                // 直接保存文件
			...
			try {
                ...
                IFile file = FileManagerFactory.getAttDocManager().createFile(name_thumb);
                // 直接保存文件
	...
	private String storeFile(FileItem item, String today) {
		if (item.isFormField() == false) { 
        	String randomName = "thumb_" + getFileName(item);
            String name = "file/"+today+"/"+ randomName;
            try {
                IFile file = FileManagerFactory.getAttDocManager().createFile(name);
                // 直接保存文件
                file.saveContent(item.get());
